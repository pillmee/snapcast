...
- 수신 메시지 타입(CODEC_HEADER / WIRE_CHUNK / TIME)별로 처리가 나뉜다.
- 이후 디코더 → 버퍼 → 플레이어 순서로 이어진다.
- 전체 흐름은 [08_control_stream_flow.md](08_control_stream_flow.md#22-클라이언트-수신-메시지-처리-message-dispatch)에 정리했다.
