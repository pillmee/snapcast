...
- Snapcast는 NTP와 비슷한 방식을 쓴다.
- 방식 이름: **왕복 지연(RTT) 기반 클록 오프셋 측정**.
- 서버가 시간을 나눠주지 않는다.
- 대신 **클라이언트가 직접** 서버와의 시간 차이(오프셋)를 잰다.
- 클라이언트는 이 오프셋으로 PCM 청크의 재생 시각을 서버 기준에 맞춘다.
...
모든 메시지의 헤더에는 타임스탬프 2개가 항상 들어있다.
...
- `tv` 구조체는 기본 생성자를 가진다.
- 이 생성자는 `steadytimeofday()`를 호출한다.
- 그래서 `tv` 객체를 만들면 현재 시각이 자동으로 채워진다.
...
- 서버는 `received`를 **자신의 로컬 시각**으로 기록한다.
- 그리고 `latency = received - sent`를 계산한다.
- 이 값을 그대로 클라이언트에 돌려보낸다(echo).
...
- POSIX에서는 `steady_clock`을 쓴다.
- `steady_clock`은 시스템 NTP 보정의 영향을 받지 않는다.
- 그래서 측정값이 흔들리지 않는다.
...
- 딜레이가 너무 길어져서 서버 시각에 못 맞출 때가 있다.
- 이때 클라이언트는 자동으로 복구한다.
- 복구는 `client/stream.cpp`의 `getPlayerChunk()`를 중심으로 3단계로 동작한다.
...
- `버퍼 설정 + 5초`보다 오래된 청크는 입력 단계에서 바로 버린다.
...
- hard_sync까지는 안 가는 작은 drift도 있다.
- 이런 작은 drift는 재생 속도를 조정해서 흡수한다.
...
- 클라이언트는 재접속 없이 스스로 복구한다.
- 늦은 청크는 버린다.
- 빈 구간은 무음으로 채운다.
- 그렇게 서버 시각으로 점프한다.
- 이후 남은 미세 drift는 샘플레이트 보정으로 흡수한다.
