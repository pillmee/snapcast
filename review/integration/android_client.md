...
- 이 문서는 그 반대 방향을 검토한다.
  ...
  - 클라이언트 역할: 원격 Snapserver의 오디오를 받아 자신의 스피커로 재생.
- 최종 목표: 하나의 Android 기기가 상황에 따라 서버도, 클라이언트도 될 수 있게 하는 것.

---

## 목표

- Android 기기가 원격 Snapserver에 접속해 오디오를 수신·재생할 수 있게 한다.
- 이미 설계된 서버 역할(HAL + Java 시스템 서비스)과 공존시킨다.

---

## 기존 자산: snapdroid 패턴이 이미 존재한다

- 검토 전에 먼저 확인할 것: "얼마나 새로 만들어야 하는가?"
- 조사 결과, **이 저장소에는 이미 성숙한 Android 클라이언트 포팅이 존재한다.**
- 이 포팅은 실제로 공식 배포 중인 Android 앱("snapdroid")에 그대로 쓰이고 있다.
...
- `Controller`(`client/controller.hpp:43`, `.cpp`)가 연결·디코더·플레이어를 총괄한다.
  - `kCodecHeader` 수신 시 디코더/스트림/플레이어 파이프라인을 전부 재구성한다.
  - `kServerSettings`로 버퍼 길이를 설정한다.
  - `kWireChunk`를 디코드해 `Stream`에 채운다(`controller.cpp:184` 이하).
- `ClientConnection`은 Snapcast 고유의 길이-프리픽스 바이너리 프로토콜을 구현한다.
  - JSON-RPC와는 별개다 — JSON-RPC는 서버의 원격 제어 API다.
- `Stream`(`client/stream.cpp`)이 시간 동기화의 핵심이다 — 아래 참고.

### 오디오 플레이어 백엔드 — Android용이 이미 2개 있다

- `client/player/player.hpp:47`에 추상 `Player`가 있다.
- 이 기반 위에 플랫폼별 백엔드가 있다.
- **Android용 백엔드가 이미 두 개 존재한다**:
...
- 두 백엔드 모두 `client/CMakeLists.txt:93-105`에서 Android 빌드 시 무조건 포함된다.
  - `oboe::oboe`/`OpenSLES`/`log`를 링크한다.
- Android 빌드는 일반 실행 파일이 아니다.
  - `libsnapclient.so`(`client/CMakeLists.txt:147-149`)로 산출되며, JNI로 로드되는 형태다.
- README에도 두 백엔드가 공식 문서화되어 있다(`README.md:122-123`).

### 시간 동기화와 레이턴시 — Android 이식에 필요한 정보가 이미 정의돼 있다

- `TimeProvider`(`client/time_provider.hpp:40`)가 서버와의 클럭 오프셋을 중앙값 필터로 유지한다.
  - `TimeProvider::serverNow()` = 로컬 시각 + 오프셋.
  - 클라이언트의 절대 시계 정확도는 중요하지 않다.
  - 중요한 것은 서버와의 **측정된 차이**뿐이다.
- 각 플레이어 백엔드는 오디오 콜백마다 지연 시간을 추정한다.
  - 추정 대상: "지금 쓴 버퍼가 실제로 스피커에서 들리기까지 걸리는 시간".
  - 이 값을 `Stream::getPlayerChunk(..., outputBufferDacTime, ...)`(`client/stream.hpp:64`)에 넘긴다.
  - `OboePlayer::getCurrentOutputLatencyMillis()`가 정확히 이 패턴이다.
  - **`AudioTrack.getTimestamp()`(프레임 위치 + `CLOCK_MONOTONIC` 나노초)는 Oboe의 `getTimestamp(CLOCK_MONOTONIC)`와 구조적으로 동일**하다.
  - 즉 Java/Kotlin `AudioTrack` 기반 플레이어를 새로 만들더라도 같은 추정 공식을 그대로 재사용할 수 있다.
- 동기화 보정은 2단계로 이뤄진다.
  - 하드 싱크: 500ms 이상 어긋나면 청크를 건너뛰거나 무음을 삽입한다(`stream.cpp:388-407`).
  - 소프트 싱크: 초당 ±0.05% 이내로 프레임을 드롭/중복해 미세 보정한다(`stream.cpp:166-243`).
  - 이 로직은 플랫폼과 무관해 그대로 재사용된다.

### 서버 디스커버리 — Android에서는 완전히 빠져 있다

- `Controller::browseMdns()`(`controller.cpp:379-416`)는 `HAS_MDNS`가 정의된 경우에만 동작한다.
- 루트 `CMakeLists.txt:209`의 `if(NOT WIN32 AND NOT ANDROID)`가 Avahi/Bonjour/mDNS 정의 블록 전체를 감싼다.
- `client/CMakeLists.txt:43`의 `elseif(NOT ANDROID)`도 Android에서는 `browse_avahi.cpp`를 아예 빌드에서 제외한다.
- **즉 오늘날의 Android 빌드는 mDNS 디스커버리가 전혀 없다.**
- 서버 주소는 외부에서(앱의 Kotlin/Java 레이어 등) 명시적으로 넘겨줘야 한다.

### 빌드/포팅 현황

- 루트 `CMakeLists.txt:51-53`가 `ANDROID`를 `MACOSX`/`WIN32`와 동급의 1급 플랫폼으로 다룬다.
  - Android는 vcpkg CONFIG 패키지로 `oboe`/`flac`/`ogg`/`opus`/`soxr`/`tremor`/`boost`를 받는다(`CMakeLists.txt:322-337`).
- `_CMakePresets.json:72-160`에 4개 ABI용 CMake 프리셋이 이미 구성돼 있다.
  - 대상: `android-x86`/`android-x86_64`/`android-armeabi-v7a`/`android-arm64-v8a`.
- `common/utils.hpp`는 Android 시스템 프로퍼티(`__system_property_get`)로 여러 함수를 구현한다.
  - 구현된 함수: `getOS()`/`getHostName()`/`getArch()`/`getHostId()`.
  - Android 6(API 23)+에서 WifiInfo MAC이 더미값(`02:00:00:00:00:00`)을 반환하는 것까지 이미 특수 처리돼 있다(`common/utils.hpp:424`).
  - `common/aixlog.hpp`도 `__android_log_write` 기반 `SinkAndroid`를 갖고 있다.
- `doc/build.md:207-218`와 `README.md:179-183`가 확인해주듯, 오늘날 실제 배포 방식은 다음과 같다.
  - 별도 저장소 [snapdroid](https://github.com/snapcast/snapdroid)가 이 저장소를 git submodule로 포함한다.
  - `libsnapclient.so`를 4개 ABI로 크로스컴파일한다.
  - 그 위에 Kotlin/Java UI + JSON-RPC 제어 + (추정) NSD 기반 디스커버리를 얹어 APK로 배포한다.
  - Google Play에 공개 배포 중인 앱이다.
...
- **오디오 재생 파이프라인(디코딩, 시간 동기화, 플레이어 백엔드)은 이미 프로덕션 검증된 상태다.**
  - 새로 설계할 필요가 없다.
- **두 개의 Android 오디오 백엔드가 이미 있다.**
  - 그중 Oboe는 `AudioTrack.getTimestamp()`와 동일한 구조의 동적 레이턴시 추정을 이미 구현했다.
  - 정확도가 검증돼 있다.
...
**권장**: HAL 통합 없이 기존 snapdroid 패턴을 그대로 재사용한다.
- 네이티브 `libsnapclient.so` + Oboe.
- 포그라운드 서비스에 JNI로 임베드.

### 검토 사항 2: 서버 역할과의 공존 (모드 전환)

- 같은 기기가 두 역할을 동시에/전환하며 수행해야 한다.
  - 서버 역할: 자체 Snapserver + HAL zone.
  - 클라이언트 역할: 원격 Snapserver 재생.
- **프로세스 관점에서는 문제 없다.**
  - `snapserver`와 클라이언트 로직은 독립 프로세스/컴포넌트다.
  - 기술적으로 동시 실행 가능하다.
- **물리적 출력은 하나뿐이라는 제약은 남는다.**
  - 상황: 이 기기의 스피커가 (a) 자신이 서버 역할로 만들어낸 zone 오디오도 재생하면서, 동시에 (b) 원격 서버의 스트림도 재생한다.
  - 이 경우 오디오가 섞이거나 충돌한다.
  - 어느 쪽이 우선인지는 **AudioManager 포커스/APM 라우팅 정책으로 중재해야 하는 제품 정책 결정 사항**이다.
  - 기술적으로 자동 해소되지 않는다.
- **미결**: 모드 전환 방식을 결정해야 한다.
...
- **오디오 포커스**
  - 다른 앱(전화, 다른 미디어 앱)이 오디오 포커스를 요청하면 시스템이 재생을 덕킹/일시정지시킨다.
  - 전용 스피커 기기와 달리 범용 Android 기기는 이 상호작용을 설계해야 한다.
  - 예: `USAGE_MEDIA`로 포커스를 잡고, 일시적 손실은 무시할지 아니면 존중해서 일시정지할지 정해야 한다.
- **멀티캐스트 제한**
  - 아래 디스커버리 이슈와 연결된다.
  - Android는 배터리 절약을 위해 기본적으로 멀티캐스트 패킷을 필터링한다.

### 검토 사항 4: 서버 디스커버리 공백

- 네이티브 mDNS가 Android에서 완전히 빠져 있다(위 "기존 자산" 참고).
- 따라서 다음 중 하나가 필요하다.
- **Android `NsdManager`**(Network Service Discovery) 사용.
  - `_snapcast._tcp` 서비스를 검색해 IP:port를 얻는다.
  - 네이티브 레이어의 `browseMdns()`를 우회한다.
  - `--host`에 해당하는 값을 직접 넘긴다.
- 멀티캐스트 기반 검색을 직접 구현하는 방법.
  - `WifiManager.MulticastLock`을 명시적으로 잡아야 한다(기본적으로 필터링됨).
- 또는 다음 방법.
  - 서버 주소를 사용자가 수동 입력하게 한다.
  - 이미 서버 역할 설계에서 만든 자바 시스템 서비스가 알고 있는 정보(같은 네트워크의 다른 Snapcast 기기 목록)를 활용한다.
